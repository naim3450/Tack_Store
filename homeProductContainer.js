...
export const homeProductContainer = (products) => {
    if (!products) {
        return false
    }
    const homeProduct = products.filter((curentElm) => {
        return curentElm.home == true
    })
    let homeProductArr = [...new Set(homeProduct)]

    homeProductArr.map((curentElm) => {
